...
public final class StateSpaceUtil {
  ...
  /**
   * Discretizes the continuous process noise covariance matrix Q using Van Loan's method in a
   * memory-allocation-free manner. Assumes the noise input matrix G = I (identity) if G is null,
   * which is common for state-space models where noise is added directly to states. Q_disc = ∫_0^dt
   * exp(A τ) G Q G^T exp(A^T τ) dτ
   *
   * <p>This implementation follows the Van Loan method: Construct V = [ -A^T, G Q G^T; 0, A ] * dt
   * Then exp(V) = [ exp(-A^T dt), exp(-A^T dt) ∫_0^dt exp(A^T τ) G Q G^T exp(A τ) dτ; 0, exp(A dt)
   * ] Extract the integral part and compute Q_disc as the transpose equivalent (symmetric).
   *
   * @param contA Continuous A (states x states)
   * @param contQ Continuous Q (states x states)
   * @param G Noise input matrix (states x noise_inputs), or null for identity
   * @param dtSeconds Time step
   * @param discQ Discretized Q (output, states x states)
   * @param V_temp Temp matrix for V (2*states x 2*states)
   * @param phi_temp Temp matrix for exp(V) (2*states x 2*states)
   * @param integral_temp Temp matrix for intermediate integral extraction (states x states)
   * @param expA_temp Pre-allocated temp for exp(A dt) (states x states)
   * @param expAT_temp Pre-allocated temp for exp(A^T dt) (states x states)
   * @param temp_GQ Temp for G Q if G provided (states x states)
   * @param temp_GQGT Temp for G Q G^T if G provided (states x states)
   */
  public static void discretizeQ(
      SimpleMatrix contA,
      SimpleMatrix contQ,
      SimpleMatrix G,
      double dtSeconds,
      ...
      SimpleMatrix V_temp,
      SimpleMatrix phi_temp,
      SimpleMatrix integral_temp,
      SimpleMatrix expA_temp,
      SimpleMatrix expAT_temp,
      SimpleMatrix temp_GQ,
      SimpleMatrix temp_GQGT) {
    ...
    if (contA.getNumRows() != states
        || contA.getNumCols() != states
        || contQ.getNumRows() != states
        || contQ.getNumCols() != states
        || discQ.getNumRows() != states
        || discQ.getNumCols() != states
        || V_temp.getNumRows() != V_size
        || V_temp.getNumCols() != V_size
        || phi_temp.getNumRows() != V_size
        || phi_temp.getNumCols() != V_size
        || integral_temp.getNumRows() != states
        || integral_temp.getNumCols() != states
        || expA_temp.getNumRows() != states
        || expA_temp.getNumCols() != states
        || expAT_temp.getNumRows() != states
        || expAT_temp.getNumCols() != states) {
      throw new IllegalArgumentException("Dimensions mismatch in discretizeQ");
    }
    ...
    // -A^T in top-left
    SimpleMatrix A_trans = contA.transpose();
    CommonOps_DDRM.scale(-1.0, A_trans.getDDRM());
    CommonOps_DDRM.insert(A_trans.getDDRM(), V_temp.getDDRM(), 0, 0);

    // Top-right: G Q G^T if G provided, else Q
    SimpleMatrix topRight;
    if (G != null) {
      if (G.getNumRows() != states || G.getNumCols() != states) {
        throw new IllegalArgumentException("G must be states x states");
      }
      if (temp_GQ.getNumRows() != states
          || temp_GQ.getNumCols() != states
          || temp_GQGT.getNumRows() != states
          || temp_GQGT.getNumCols() != states) {
        throw new IllegalArgumentException("temp_GQ and temp_GQGT must be states x states");
      }
      // temp_GQ = G * Q
      CommonOps_DDRM.mult(G.getDDRM(), contQ.getDDRM(), temp_GQ.getDDRM());
      // temp_GQGT = temp_GQ * G^T
      CommonOps_DDRM.multTransB(temp_GQ.getDDRM(), G.getDDRM(), temp_GQGT.getDDRM());
      topRight = temp_GQGT;
    } else {
      topRight = contQ;
    }
    CommonOps_DDRM.insert(topRight.getDDRM(), V_temp.getDDRM(), 0, states);

    // A in bottom-right
    CommonOps_DDRM.insert(contA.getDDRM(), V_temp.getDDRM(), states, states);

    // V = V * dt (in-place scaling)
    CommonOps_DDRM.scale(dtSeconds, V_temp.getDDRM());

    // phi = exp(V)
    EigenJNI.exp(V_temp.getDDRM().getData(), V_size, phi_temp.getDDRM().getData());

    // Extract exp(A dt) from bottom-right (phi[states:, states:])
    CommonOps_DDRM.extract(
        phi_temp.getDDRM(), states, V_size, states, V_size, expA_temp.getDDRM(), 0, 0);

    // Extract the integral part from top-right (phi[0:states, states:]) = exp(-A^T dt) * ∫ exp(A^T
    // τ) (G Q G^T) exp(A τ) dτ
    CommonOps_DDRM.extract(
        phi_temp.getDDRM(), 0, states, states, V_size, integral_temp.getDDRM(), 0, 0);

    // Compute ∫ exp(A^T τ) (G Q G^T) exp(A τ) dτ = exp(A^T dt) * integral_temp
    // exp(A^T dt) = transpose(expA_temp)
    CommonOps_DDRM.transpose(expA_temp.getDDRM(), expAT_temp.getDDRM());

    // Q_disc = exp(A^T dt) * integral_temp
    CommonOps_DDRM.mult(expAT_temp.getDDRM(), integral_temp.getDDRM(), discQ.getDDRM());

    // Note: The computed Q_disc is actually ∫ exp(A^T τ) (G Q G^T) exp(A τ) dτ, but since Q is
    // symmetric and the integral is symmetric,
    // it equals [∫ exp(A τ) (G Q G^T) exp(A^T τ) dτ]^T, which is equivalent for use in the filter.
  }

  /**
   * Overload for discretizeQ with G = I (identity), no need for temp_GQ and temp_GQGT.
   *
   * @param contA Continuous A (states x states)
   * @param contQ Continuous Q (states x states)
   * @param dtSeconds Time step
  ...
  /**
   * Discretizes the continuous measurement noise covariance matrix R. For white noise measurements,
   * the discrete R is approximately R_cont / dtSeconds, as the variance scales inversely with the
   * sampling rate.
   *
   * @param contR Continuous R (outputs x outputs)
   * @param dtSeconds Time step
   * @param discR Discretized R (output, outputs x outputs)
   */
  public static void discretizeR(SimpleMatrix contR, double dtSeconds, SimpleMatrix discR) {

    int outputs = contR.getNumRows();

    // Runtime size check
    if (contR.getNumCols() != outputs
        || discR.getNumRows() != outputs
        || discR.getNumCols() != outputs) {
      throw new IllegalArgumentException("Dimensions mismatch in discretizeR");
    }
    ...
    // Check R positive definite using SVD
    SingularValueDecomposition_F64<DMatrixRMaj> svdR =
        DecompositionFactory_DDRM.svd(outputs, outputs, false, false, true);
    svdR.decompose(contR.getDDRM());
    ...
    // discR = contR / dtSeconds (in-place if possible, but copy to output)
    System.arraycopy(
        contR.getDDRM().getData(), 0, discR.getDDRM().getData(), 0, contR.getNumElements());
    CommonOps_DDRM.scale(1.0 / dtSeconds, discR.getDDRM());
  }
